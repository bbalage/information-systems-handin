...
export class camelCase1618585323413 implements MigrationInterface {
    name = 'camelCase1618585323413'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query("CREATE TABLE `borrowable` (`id` int NOT NULL AUTO_INCREMENT, `title` varchar(100) NOT NULL, `author` varchar(100) NOT NULL, `maxBorrowTime` int NOT NULL, `type` char(1) NOT NULL, PRIMARY KEY (`id`)) ENGINE=InnoDB");
        await queryRunner.query("CREATE TABLE `borrowable_item` (`serialNumber` int NOT NULL AUTO_INCREMENT, `acquirementDate` date NOT NULL, `status` char(1) NOT NULL, `borrowableId` int NULL, PRIMARY KEY (`serialNumber`)) ENGINE=InnoDB");
        ...
        await queryRunner.query("CREATE TABLE `borrow` (`borrowId` int NOT NULL AUTO_INCREMENT, `dateOfBorrow` date NOT NULL, `memberId` int NULL, `borrowableItemSerialNumber` int NULL, PRIMARY KEY (`borrowId`)) ENGINE=InnoDB");
        ...
        await queryRunner.query("ALTER TABLE `borrow` ADD CONSTRAINT `FK_342b59afd651f54763c836d5bb3` FOREIGN KEY (`borrowableItemSerialNumber`) REFERENCES `borrowable_item`(`serialNumber`) ON DELETE NO ACTION ON UPDATE NO ACTION");
    }
